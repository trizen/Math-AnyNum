...
# A generalized algorithm with O(sqrt(n)) complexity for computing the partial-sums of the `sigma_j(k)` function:
#
...
use Math::AnyNum qw(faulhaber_sum bernoulli isqrt ipow);
...
foreach my $m (0 .. 10) {
    ...
    my $t1 = faulhaber_partial_sum_of_sigma($n, $m);
    my $t2 = bernoulli_partial_sum_of_sigma($n, $m);

    say "Sum_{k=1..$n} sigma_$m(k) = $t1";

    die "error: $t1 != $t2" if ($t1 != $t2);
}

die "error" if faulhaber_partial_sum_of_sigma(10**8, 2) != "400685641565621401132515";
