...

{
    ...
    like(
        Math::AnyNum->new('-1.2')**Math::AnyNum->new('3.4'),
        qr/^-0\.5743790627709710900290932\d*-1\.767756985588369210401232327481\d*i\z/
    );

    like(
        (-$z)**(1/$q),
        qr/^1\.7320508075688772935274463415058723669\d*i\z/
    );

    like(
        (-$q)**(-1/$q),
        qr/^-0\.70710678118654752440084436210484903928\d*i\z/
    );

    like(
        (-$q)**(-1/$z),
        qr/^0\.39685026299204986868792640981\d*-0\.6873648184993013131917395984430\d*i\z/
    );
    ...
    like(
        (-$z)**(-1/$f),
        qr/^0\.537284965911770959776669078352965\d*-0\.53728496591177095977666907835296\d*i\z/
    );

    like(
        (-$q)**(-1/$f),
        qr/^0\.5946035575013605333587499852\d*\-0\.59460355750136053335874998528023\d*i\z/
    );

    like(
        (-$q)**(1/$f),
        qr/^0\.8408964152537145430311254762332\d*\+0\.840896415253714543031125\d*i\z/
    );
    ...
    #
    like(
        (($z + $f*sqrt($q-$z))-sqrt(-$z+$q)*$f)->zeta,
        qr/^1\.202056903159594285399738161511449\d*\z/
    );

    like(
        (($z + $f*sqrt($q-$z))-sqrt(-$z+$q)*($c-1))->zeta,
        qr/^1\.202056903159594285399738161511449\d*\z/
    );

    like(
        (($z + $f*sqrt($q-$z)) - ($z+1)*sqrt(-$z+$q))->zeta,
        qr/^1\.202056903159594285399738161511449\d*\z/
    );

    like(
        (($z + $f*sqrt($q-$z)) - sqrt(-$z+$q)*($z+1))->zeta,
        qr/^1\.202056903159594285399738161511449\d*\z/
    );

    like(
        (($z + $f*sqrt($q-$z)) - sqrt(-$z+$q)*($q+2))->zeta,
        qr/^1\.202056903159594285399738161511449\d*\z/
    );
