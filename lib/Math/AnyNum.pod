...
=head2 frac_approx
    ...
    frac_approx(n)                                #=> Rat | NaN

Given a real number C<n>, it returns the smallest fraction approximation for C<n>, which is found using continued fractions.

Example:

    frac_approx(3.14)  = 22/7
    frac_approx(1.618) = 89/55
