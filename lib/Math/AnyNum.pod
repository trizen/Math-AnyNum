...
Math::AnyNum provides a transparent interface to L<Math::GMPz>, L<Math::GMPq>, L<Math::MPFR> and L<Math::MPC>,
focusing on performance and easy-to-use. In most cases, it can be used as a drop-in replacement for the
L<bignum> and L<bigrat> pragmas.

The philosophy of Math::AnyNum is that mathematical operations should just work, no matter what, therefore the support
for complex number is virtually transparent without requiring any explicit conversions. The conversions are done implicitly,
using a sophisticated promotion system, that tries really hard to do the right thing and as efficiently as possible.

In addition, any Math::AnyNum object is B<immutable>.
...
The original project was called L<Math::BigNum>, but because of some design flaws, it was very hard to integrate
support for complex numbers, therefore the project was abandoned and much of its code ended up in this module.

=head1 HOW IT WORKS

Math::AnyNum tries really hard to do the right thing and as efficiently as possible.
For example, when computing C<pow(x, y)>, it first checks the types of C<x> and C<y> to see if they are
integers, so it can optimize the operation to an integer exponentiation by calling the corresponding
I<mpz> function. When C<x> is a rational number and C<y> is an integer, it does rational exponentiation,
which computes the exact result. Otherwise, it will fallback to floating-point exponentiation, using the
corresponding I<mpfr> or I<mpc> function, depending on the type of the internal object, which can have one
of the following types: Math::GMPz, Math::GMPq, Math::MPFR or Math::MPC.
...
The following functions are exportable:
    ...
    :misc
        irand iseed mod floor ceil round sign
        ...
        real imag numerator denominator int rat
        float complex
...
The integer square root of C<n> and the integer cube root of C<n>. Returns NaN when C<n> is negative.
...
Returns a true value when C<x> is a real number (i.e.: when the imaginary part is zero and it holds a regular value different of +/-Inf and NaN).
...
Returns a true value when C<n> is divisible by 2.

=head2 is_odd

    is_odd(n)                                     #=> Bool

Returns a true value when C<n> is not divisible by 2.
