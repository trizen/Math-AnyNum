...
=head1 FUNCTIONS
  ...
  :special
    ...
    polymod(@list)               Poly modulo function
  ...
  :misc
    ...
    float(x)            convert x into a floating-point number
    complex(x)          convert x into a complex number
    ...
    acmp(a,b,k)         approximate comparison of round(a,k) and round(b,k)
...
to 62 inclusive and defaults to 10.
...
Returns the n-th Bernoulli number C<B_n> as an exact fraction, starting with C<bernoulli(0) = 1>.
...
Returns the n-th Bernoulli number, as a floating-point approximation, with C<bernreal(0) = 1>.
...
Returns the n-th Catalan numbers.
