...
=head1 FUNCTIONS
  ...
  :ntheory
    ...
    chebyshevT(n,x)           Chebyshev polynomial of the 1st kind: T_n(x)
    chebyshevU(n,x)           Chebyshev polynomial of the 2nd kind: U_n(x)
...
Compute the Chebyshev polynomial of the first kind: C<T_n(x)>, where C<n> must be a native integer.
...
Compute the Chebyshev polynomial of the second kind: C<U_n(x)>, where C<n> must be a native integer.
